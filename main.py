...
class DataHandler:
    ...
    def get_hws_from_data_files(self, button=False):
        """通过数据接口收到的数据获取作业数据"""
        if button:
            self.request_data_and_save_into_files()
        elif os.path.isfile(self.data_path) is False or os.path.isfile(self.id2name_path) is False:
            self.request_data_and_save_into_files()
...
def main():
    ...
    homeworks = data_handler.get_hws_from_data_files(button=(True if sys_len >= 4 and sys.argv[3] == "1" else False))
